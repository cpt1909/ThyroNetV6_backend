...
def download_from_gdrive(file_id, output_path):
    if not os.path.exists(output_path):
        print("[INFO] Downloading ThyroidClassifierV6.pth from Google Drive...")
        url = f"https://drive.google.com/uc?id={file_id}"
        try:
            gdown.download(url, output_path, quiet=False)
            print("[SUCCESS] Model downloaded and saved to:", output_path)
...
def load_model_1(model, checkpoint_path):
    ...
    file_id = "1xZYGUugmmucMolNRgX83_EktSgmUNkzs"
    
    download_from_gdrive(file_id, checkpoint_path)
